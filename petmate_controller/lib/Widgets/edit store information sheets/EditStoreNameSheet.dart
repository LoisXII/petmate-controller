...
class EditStoreNameSheet extends StatelessWidget {
  const EditStoreNameSheet({super.key});
        ...
        builder: (context, constraints) => Column(
          ...
          children: [
            ...
            Container(
              ...
              child: AutoSizeText(
                MyServices().capitalizeEachWord('update store name'),
            ...
            Consumer<LocalFunctionProvider>(
              builder: (context, functions, child) => MyTextFormFieldTemplate(
                ...
                border_color: color4,
                controller: functions.store_name_controller_upadte,
                text: MyServices().capitalizeEachWord('new store name '),
            ...
            Consumer<LocalFunctionProvider>(
              builder: (context, functions, child) =>
                ...
                builder: (context, firebase, child) => MyButtonTemplate(
                    ...
                    onPressed: () async {
                      if (functions
                          .store_name_controller_upadte.text.isNotEmpty) {
                        // show loading .
                        MyServices().ShowLoading(context: context);

                        // update .
                        await firebase.UpdateStoreName(
                                store_name: functions
                                    .store_name_controller_upadte.text
                                    .toLowerCase()
                                    .trim())
                            .whenComplete(
                          () {
                            ...
                            MyServices().ShowNotificationSheet(
                              ...
                              message: MyServices().capitalizeEachWord(
                                  'update the store name has been done successfully !'),
                              action: () {},
                            );
                          },
                        );
                      } else {
                        print("the new store name textfield is empty ! ");
                      }
                    },
                    style: Theme.of(context).textTheme.headlineLarge!.copyWith(
                          fontWeight: FontWeight.bold,
